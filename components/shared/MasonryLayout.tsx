...
export default function MasonryLayout({ memes }: MasonryLayoutProps) {
  return (
    ...
    {memes.length === 0 && <p className="text-gray-500 text-base font-bold text-center">No memes found</p>}
    {memes && memes.map((meme: Meme, index) => (
      <Fragment key={meme._id}>
       <Card key={meme._id}>
       <CardBody className={`w-full ${ index % 2 ? 'h-[200px]' : 'h-[300px]'} flex flex-col gap-3 overflow-hidden`}>
        {meme.file && (
